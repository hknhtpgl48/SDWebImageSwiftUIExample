    ...
    var body: some View {
        NavigationStack {
            GeometryReader { geo in
                ScrollView {
                    LazyVStack {
                        ForEach(photos) { photo in
                            AsyncImage(url: URL(string: photo.download_url.absoluteString)) { image in
                                image
                            ...
                            .padding()
                            .scrollTransition(topLeading: .identity, bottomTrailing: .interactive) { view, transition in
                ...
                .padding()
                .onAppear {
                    self.photos = decodeImageData2()
                    UIScrollView.appearance().bounces = false//To stop scrollView going over toolbar
                }
            }
